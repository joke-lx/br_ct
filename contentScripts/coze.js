...
function recycleResponseListener(reason) {
  const listener = window.__responseListenerInstances && window.__responseListenerInstances.coze;
  if (!listener || typeof listener.reset !== 'function') return;
  logInfo(`Coze 手动发送，回收回复监听: ${reason}`);
  listener.reset();
}
...
if (!window.location.hostname.includes(PLATFORM_CONFIG.hostname)) {
  ...
  if (!window.__cozeManualRecycleBound) {
    window.__cozeManualRecycleBound = true;
