...
function recycleResponseListener(reason) {
  const listener = window.__responseListenerInstances && window.__responseListenerInstances.tongyi;
  if (!listener || typeof listener.reset !== "function") return;
  console.log(`通义千问 手动发送，回收回复监听: ${reason}`);
  listener.reset();
}
...
if (!window.location.hostname.includes("qianwen")) {
  ...
  if (!window.__tongyiManualRecycleBound) {
    window.__tongyiManualRecycleBound = true;
