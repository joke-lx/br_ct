...
function recycleResponseListener(reason) {
  const listener = window.__responseListenerInstances && window.__responseListenerInstances.googlestudio;
  if (!listener || typeof listener.reset !== "function") return;
  console.log(`Google AI Studio 手动发送，回收回复监听: ${reason}`);
  listener.reset();
}
...
if (!window.location.hostname.includes(targetHostname)) {
  ...
  if (!window.__googlestudioManualRecycleBound) {
    window.__googlestudioManualRecycleBound = true;
