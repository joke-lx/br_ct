...
**⚠️ transient activation 限制（重要）**：
`document.execCommand('copy')` 需要 **transient activation**（用户真实手势）。`dispatchEvent` 产生的 `isTrusted: false` 事件**不满足**这一条件。因此：

- 模拟点击（`dispatchEvent`/`click()`）触发平台的 click handler → handler 调用 `execCommand('copy')` → Chrome 拒绝（无 TA）→ **copy 事件根本不触发**
- 路径 B 的 capture 阶段 copy 事件监听器在这些场景下**完全不会执行**
- 路径 B 的 Selection fallback 仅对**真实用户点击**有效

**适用条件**：真实用户点击场景下，拦截使用 `execCommand('copy')` 且通过 textarea 方式复制的平台。只能拿纯文本，HTML 质量不可控。
...
路径 B（copy event）:
  click → execCommand('copy') → [transient activation 校验] → copy 事件触发 → [capture handler] → postMessage
                                              ↓ dispatchEvent 无 TA → 事件不触发 → ✗ 静默失败
...
| 维度 | clipboard.write 原型替换（路径 A） | copy 事件 + Selection（路径 B） | execCommand 方法级替换（路径 C） |
|------|------------------------|------------------------|------------------------|
| 拦截可靠性 | ★★★★★ — 安装即拦截所有调用 | ★☆☆☆☆ — 依赖 TA，dispatchEvent 模拟时**不触发** | ★★★★★ — 安装即拦截所有 execCommand('copy') 调用 |
| HTML 完整度 | ★★★★★ — text/html 原生格式 | ★☆☆☆☆ — textarea 无 HTML，Selection 可能有但不完整 | ★★★☆☆ — Selection 数据，textarea 场景无 HTML |
| isTrusted 依赖 | ❌ 不需要 | ✅ **需要**（无 TA 时 copy 事件不触发） | ❌ 不需要（在 TA 校验之前拦截） |
| TA 依赖 | ❌ 不需要 | ✅ 需要真实用户手势 | ❌ 不需要（同线程同步，早于浏览器校验）|
| 框架影响 | ❌ 无影响 | ✅ 平台 UI 变化可能打破 Selection fallback | ❌ 无影响 |
| 一次性适配 | ✅ 全部平台一次解决 | ❌ 每个平台需要单独看实现细节 | ✅ 全部 execCommand 平台一次解决 |
| 当前适用 | Gemini、GPT、Claude、Grok、DS | 真实点击场景的旧平台 | **GLM（智谱清言）**、其他 execCommand 平台 |
...
- [ ] 若使用 execCommand：
  - [ ] dispatchEvent 模拟点击 → 检查 copy 事件是否触发（路径 B 仅对真实点击有效）
  - [ ] CDP 验证：拦截 execCommand，检查 `document.activeElement.tagName` 和 `window.getSelection().toString()` 是否包含数据
  - [ ] 路径 C（execCommand hook）是否已注入（mainWorldHook.js 中查找 `_origExecCommand`）
...
- [ ] 确认三条路径覆盖关系：
  ...
  - 路径 B（copy 事件 capture）→ 真实用户点击场景的补充
