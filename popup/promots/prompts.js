const PROMPT_TEMPLATES = {
  ...
  指令序列: {
    ...
    label: "docker运行",
    template:
      "主题: %s  要求: 1. 生成一个最小运行的docker run 一行指令,优先保证运行,数据卷都使用数据卷名进行映射,不要指定路径,其他的用户密码优先使用默认配置,如果需要指定 都使用容器作为用户名 223456作为密码 保证快速启动  2. 生成完整的docker-compose.yml文件 可以包含一些高级的参数 比如各种卷的挂载(使用./xxxx进行相对路径的挂载),环境变量的设置,具有良好的配置管理 . 完成上面两个主题的配置 ",
  },
  "有问题:: 官方文档 sdk文档 ": {
    group: "search",
