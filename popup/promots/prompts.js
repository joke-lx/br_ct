const PROMPT_TEMPLATES = {
  ...
  数据结构分析: {
    ...
    label: "数据结构分析",
    template:
      "内容： %s  要求 ： 深入分析这个数据结构的设计原理 和底层实现，比如支持功能的最小范围所需要的字段， 以及在实际应用中的优势和劣势 ，并且把这个数据结构提供的相关方法抽取成单独的接口文件，每个接口上面进行详细的举例和描述，进行彻底详细的注释，实现接口即文档的思想，一个接口的方法需要包含： 基本功能描述，入参出参含义以及其中可能使用到的堆栈和扩展点",
  },
  // 内容转译类
  相关网站搜索: {
    group: "search",
